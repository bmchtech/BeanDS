...
final class GeometryEngine {
    struct MatrixStack(int size) {
        ...
        Mat4x4 peek() {
            log_gpu3d("MATRIX: PEEK. %f", stack_pointer);
            if (stack_pointer == 0) return Mat4x4.identity();
    ...
    void multiply_matrix(Mat4x4 matrix) {
        final switch (matrix_mode) {
            ...
            case MatrixMode.TEXTURE:
                log_gpu3d("how does texture stack work?");
                break;
        }
    }
    ...
    void handle_VTX_16(Word* args) {
        switch (polygon_type) {
            case PolygonType.TRIANGLES:
                ...
                v = (projection_matrix * modelview_matrix) * v;

                if (triangle_assembler.submit_vertex(
                    Vertex(v, current_color_r, current_color_g, current_color_b)
                )) {
                    parent.geometry_buffer[polygon_index] = triangle_assembler.get_polygon();
                    polygon_index++;
                }
                break;
            
            default:
                error_gpu3d("Unimplemented polygon type: %x", polygon_type);
        }
    }
...
static GPU3DCommand[256] generate_commands()() {
    ...
    commands[0x13] = GPU3DCommand("MTX_STORE",        0x13, 1,  17,  false);
    commands[0x14] = GPU3DCommand("MTX_RESTORE",      0x14, 1,  36,  false);
    commands[0x15] = GPU3DCommand("MTX_IDENTITY",     0x15, 0,  19,  true);
    commands[0x16] = GPU3DCommand("MTX_LOAD_4x4",     0x16, 16, 34,  false);
    commands[0x17] = GPU3DCommand("MTX_LOAD_4x3",     0x17, 12, 30,  false);
    ...
    commands[0x21] = GPU3DCommand("NORMAL",           0x21, 1,  9,   false);
    commands[0x22] = GPU3DCommand("TEXCOORD",         0x22, 1,  1,   false);
    commands[0x23] = GPU3DCommand("VTX_16",           0x23, 2,  9,   true);
    commands[0x24] = GPU3DCommand("VTX_10",           0x24, 1,  8,   false);
    commands[0x25] = GPU3DCommand("VTX_XY",           0x25, 1,  8,   false);
    commands[0x26] = GPU3DCommand("VTX_XZ",           0x26, 1,  8,   false);
    commands[0x27] = GPU3DCommand("VTX_YZ",           0x27, 1,  8,   false);
    commands[0x28] = GPU3DCommand("VTX_DIFF",         0x28, 1,  8,   false);
    commands[0x29] = GPU3DCommand("POLYGON_ATTR",     0x29, 1,  1,   true);
    commands[0x2A] = GPU3DCommand("TEXIMAGE_PARAM",   0x2A, 1,  1,   false);
    commands[0x2B] = GPU3DCommand("PLTT_BASE",        0x2B, 1,  1,   false);
